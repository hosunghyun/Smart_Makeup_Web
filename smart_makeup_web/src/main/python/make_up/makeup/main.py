...
class start_Makeup():
    def __init__(self, config ):
        ...
        self.s_mix = config.s_mix
        self.l_mix = config.l_mix
        ...
        if self.skin_val or self.lip_val:
            ...
            if self.skin_val:
                if self.skin_name:
                    ...
                    skin = makeup_face()
                    skin_result, skin_mask = skin.run(frame,skin_path, self.s_mix)
                else:
                    print("피부 파일이 없습니다.")
                    # return output
            if self.lip_val:
                if self.lip_name:
                    ...
                    lip = makeup_lips()
                    lip_result, lip_mask = lip.run(frame,lip_path, self.l_mix)         
                else:
                    print("입술 파일이 없습니다.")
                    # return output
